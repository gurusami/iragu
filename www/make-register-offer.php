...
class IraguMakeRegisterOffer extends IraguWebapp {
   ...
   public $offer_from = "";
   public $offer_to   = "";
   public $cash_back  = ""; /* in paise */
   public $reason     = "";
   public $offer_by   = "";

   public function getCashBack() {
       return number_format((float) $this->cash_back / 100, 2, '.', '');
   }
...
EOF;
     ...
     while ($row = $result->fetch_assoc()) {
         $cash = number_format((float) $row['cash_back'] / 100, 2, '.', '');
   ...
   public function check_offer_dates() {
     ...
     $this->success = $stmt->bind_param('ss', $this->offer_from,
                                              $this->offer_to);
     if (!$this->success) {
    ...
    <legend> Make a Register Offer </legend>
      ...
      <p> <label for="cash_back"> Cash Back </label>
          <input type="number" step="0.01" id="cash_back" name="cash_back"
                 value="<?php echo $page->getCashBack(); ?>"
                 maxlength="8"/> </p>
